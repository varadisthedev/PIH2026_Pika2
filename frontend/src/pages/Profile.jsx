...
import {
    ...
    Calendar, Bell, CreditCard, LogOut,
    Package, ChevronRight, Camera
} from 'lucide-react';
...
export default function Profile() {
    ...
    if (!isLoaded) return <div className="min-h-screen flex items-center justify-center"><div className="w-8 h-8 border-2 border-brand-green border-t-transparent rounded-full animate-spin" /></div>;

    const displayName = user ? `${user.firstName || ''} ${user.lastName || ''}`.trim() || user.username || 'RentiGO User' : 'RentiGO User';
    const displayEmail = user?.primaryEmailAddress?.emailAddress || 'Not available';
    const avatarUrl = user?.imageUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${displayName}`;
    const joinDate = user?.createdAt ? new Date(user.createdAt).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' }) : 'Unknown';
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[400px_1fr] gap-12 items-start">
                        ...
                        <Card variant="glass" className="!p-10 !rounded-[3rem] text-center space-y-8 border-brand-green/20">
                            ...
                            <div className="space-y-4">
                                <div className="space-y-1">
                                    <h2 className="text-3xl font-black text-brand-dark dark:text-brand-frost tracking-tighter uppercase leading-none">{displayName}</h2>
                                ...
                                <div className="text-xs font-black text-brand-teal/60 uppercase tracking-widest">
                                    {displayEmail}
                                </div>
                            </div>

                            <div className="pt-8 border-t border-brand-teal/5 flex gap-4">
                                <Button variant="primary" className="flex-1 !rounded-[2rem]" onClick={() => user?.openUserProfile?.()}>
                                    Edit Profile
                                </Button>
                            </div>
                        </Card>
                    ...
                    <div className="space-y-12">
                        ...
                        <section className="space-y-8 animate-fade-up">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {[
                                    ...
                                    { icon: Calendar, label: 'Joined Neighborhood', value: joinDate },
                                ].map((item, i) => (
                        ...
                        <section className="pt-12 border-t border-brand-teal/5">
                                <Button variant="ghost" className="!text-red-500 !text-xs font-black uppercase tracking-[0.2em] px-8" onClick={() => signOut(() => navigate('/'))}>
